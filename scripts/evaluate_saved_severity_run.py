...
from traffic_incident_cv.evaluation.severity_post_eval import (
    build_prediction_rows,
    export_severity_post_eval_bundle,
)
...
def _resolve_manifest_path(run_dir: Path, manifest_path: str, output_dir_name: str) -> Path:
    ...
    if path.is_absolute():
        ...
        candidates.extend([Path.cwd() / path, run_dir / path, run_dir.parent / path])
...
def _build_loader(run_dir: Path, config: dict[str, Any], split: str) -> tuple[Any, list[Any], Path, str]:
    ...
    return loader, records, resolved_manifest_path, eval_sampler_mode
...
def evaluate_saved_run(
    *,
    ...
    loader, records, resolved_manifest_path, eval_sampler_mode = _build_loader(run_path, config, split)
    ...
    metrics_payload = {
        ...
        "metadata": {
            **bundle.metadata,
            ...
            "eval_clip_sampler": eval_sampler_mode,
            "labels": labels,
        },
    }
    export_payload = export_severity_post_eval_bundle(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate saved severity run on a requested split.")
